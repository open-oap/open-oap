...
//        $html[]=               GeneralUtility::implodeAttributes($attributes, true);
    ...
    private function initialize(array $params): void
    {
        ...
        $model = $params['config']['itemsProcConfig']['model'];
        $pidKey = $params['config']['itemsProcConfig']['pidRoot'];
        $pidRoot = $typoScript['plugin.']['tx_openoap_dashboard.']['settings.'][$pidKey];

        $queryGenerator = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance( 'TYPO3\\CMS\\Core\\Database\\QueryGenerator' );
        $childPidsString = $queryGenerator->getTreeList($pidRoot, $this->depth, 0, 1); //Will be a string like 1,2,3
    ...
    public function getPoolPages(array &$params): void
    {
        ...
        $proposalsPoolRoot = $typoScript['plugin.']['tx_openoap_dashboard.']['settings.']['proposalsPoolId'];
        ...
        $queryGenerator = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance( 'TYPO3\\CMS\\Core\\Database\\QueryGenerator' );
        $childPids = $queryGenerator->getTreeList($proposalsPoolRoot, $this->depth, 0, 1); //Will be a string like 1,2,3
